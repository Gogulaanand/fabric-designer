...
export function ExportDialog({ onExportPng, onExportJpg, onCopyToClipboard, hasImage }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="text-sm text-slate-400 font-semibold uppercase tracking-wider">Export</div>
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-2">
          <span className="text-sm text-slate-500">JPG Quality</span>
          <input
          ...
          />
          <span className="text-sm text-slate-500 w-10 text-right font-mono">{Math.round(jpgQuality * 100)}%</span>
        </div>
        <button
